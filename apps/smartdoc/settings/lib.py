...
# celery相关配置
celery_data_dir = os.path.join(PROJECT_DIR, 'data', 'celery_task').replace('\\', '/')
if not os.path.exists(celery_data_dir) or not os.path.isdir(celery_data_dir):
    os.makedirs(celery_data_dir)
broker_path = os.path.join(celery_data_dir, "celery_db.sqlite3").replace('\\', '/')
backend_path = os.path.join(celery_data_dir, "celery_results.sqlite3").replace('\\', '/')
